...
const fetchLoggedFoodsForDateRange = async (userId: string, startDate: Date, endDate: Date): Promise<LoggedFood[]> => {
...
export const useLoggedFoodsForDateRange = (startDate: Date, endDate: Date) => {
  const { user } = useAuth();

  return useQuery<LoggedFood[], Error>({
    queryKey: ['loggedFoodsForDateRange', user?.id, format(startDate, 'yyyy-MM-dd'), format(endDate, 'yyyy-MM-dd')],
    queryFn: () => {
      if (!user) return Promise.resolve([]);
      return fetchLoggedFoodsForDateRange(user.id, startDate, endDate);
    },
    enabled: !!user,
